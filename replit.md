...
-   **Excel Import Module**: Bulk import functionality for players and matches (Singles/Doubles) with automatic data processing, player creation, and tournament registration. It includes downloadable templates, Zod validation with detailed error reporting, and a results preview. The system includes automatic category mapping that accepts both friendly names ("PRO Singles IRT", "Amateur A") and database enum values ("PRO_SINGLES_IRT", "AMATEUR_A") from Excel files, providing clear error messages when invalid categories are detected. Authorization is restricted to SuperAdmin and Tournament Admin roles.
...
-   **Match and Player Categories**: Comprehensive categorization system with 13 official competition categories. Players can be assigned up to 3 categories and matches can be categorized during creation. Categories include: PRO Singles IRT, Dobles Open, Amateur A/B/C, Principiantes, Juvenil 18 y menores (Varonil/Femenil), Dobles AB/BC, Master 35+/55+, and Dobles Master 35+. Category badges are displayed in match listings and user tables. The system supports multiple category selection with frontend and backend validation (maximum 3 categories per player).
-   **Player Profile Enhancement**: Players can now have a profile photo, nationality (with flag display), and up to 3 assigned categories. **Direct Photo Upload**: Integrated file upload functionality allows users to upload profile photos directly from their laptop or mobile device. The system uses Replit Object Storage (Google Cloud Storage) with server-side validation (5MB max, image types: JPEG, PNG, WEBP, GIF). Users can either upload a file or provide a URL. The PhotoUploader component includes live preview, loading states, and automatic query invalidation. Backend endpoint `/api/media/profile-photo` handles multipart uploads with Multer middleware, validating file type and size before storing in the public bucket. User management interface shows player photos, country flags, and multiple category badges in a visually enhanced table format.
-   **Real-Time Public Display System**: Live scoreboard display system accessible without authentication at `/public-display` for showing match scores on external monitors or projectors. Features include: (1) **WebSocket Real-Time Updates**: Dual-channel WebSocket architecture with protected stats capture channel (`/ws/match-stats`) requiring authentication and public display channel (`/ws/public-display`) for read-only live updates. Single WebSocket server multiplexes both channels with throttled broadcasts (250ms coalescing) to prevent flooding. Automatic reconnection with 3-second backoff ensures continuous operation. (2) **Automatic Match Rotation**: Displays active matches from database with configurable rotation interval per tournament (default 40 seconds). Supports filtering by tournament via `?tournament=tournamentId` query parameter or showing all active matches across all tournaments. (3) **Player Information Display**: Shows player names, profile photos (with avatar fallback), and nationality flags. Supports both singles and doubles matches with proper team structure (player1+player3 vs player2+player4). (4) **Live Score Display**: Real-time scoreboard with current set indicator, serve tracking (animated yellow dot), all three sets' scores with color-coded highlighting, and match statistics (technical fouls, timeouts, appellations). (5) **Sponsor Banner System**: Rotating sponsor banner at bottom of screen changing every 5 seconds. Tournament administrators can manage sponsors via `SponsorsManagement` component with CRUD operations including sponsor name, logo URL, website link, and display order. Database table `tournamentSponsors` stores sponsor information with `isActive` flag and `displayOrder` field for rotation control. Backend sanitizes all public WebSocket payloads to remove sensitive data, ensuring security while maintaining real-time updates for spectators.

## WebSocket Architecture
The system implements a unified WebSocket server with two separate channels:
- **Stats Capture Channel** (`/ws/match-stats`): Protected channel requiring session authentication for real-time updates to match officials capturing statistics. Clients join specific match rooms by matchId.
- **Public Display Channel** (`/ws/public-display`): Public channel (no authentication) for read-only scoreboard displays. Supports tournament filtering and receives throttled, sanitized match updates.

All match state changes broadcast to both channels: immediate updates to match rooms and throttled (250ms) sanitized updates to public displays. Heartbeat mechanism (30s ping/pong) maintains connection health with automatic cleanup of stale connections.
